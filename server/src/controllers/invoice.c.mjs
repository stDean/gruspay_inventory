...
export const InvoiceCtrl = {
	...
	getInvoice: async (req, res) => {
		...
		if (invoice.status === "SWAP") {
			...
			// List outgoing products (itemsPurchased) without grouping
			itemsPurchased = swap.outgoingProducts.map(product => ({
				name: product.product_name,
			...
			// List incoming products (incomingItems) without grouping
			incomingItems = swap.incomingProducts.map(product => ({
				name: product.product_name,
				serialNo: product.serial_no,
				desc: product.description,
				price: parseFloat(product.price),
			...
			// Regular invoice items (not swap)
			itemsPurchased = invoice.product.map(product => ({
				name: product.product_name,
				serialNo: product.serial_no,
				desc: product.description,
				price: parseFloat(product.bought_for),
